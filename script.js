...
// Improved lookup: use fuzzy match automatically and show closest match instead of "Not found"

// Improved lookup: suggest closest match via small UI instead of confirm()
window.lookupFoodByName = function(q){
  try{
    if(!q) { alert('Please type a food name'); return null; }
    const raw = q.trim().toLowerCase();
    // try exact
    if(window.foodData && window.foodData[raw]){
      window.showFoodData(raw);
      return raw;
    }
    // try substring matches
    for(const k of Object.keys(window.foodData || {})){
      if(k.includes(raw) || raw.includes(k)){
        window.showFoodData(k);
        return k;
      }
    }
    // fallback to fuzzyFind if available
    let fuzzyKey = null;
    if(typeof fuzzyFind === 'function'){
      fuzzyKey = fuzzyFind(raw);
    } else if(typeof window.findMatch === 'function'){
      fuzzyKey = window.findMatch(raw);
    }
    const suggestionBox = document.getElementById('suggestionBox');
    if(fuzzyKey){
      // show a small suggestion UI (Did you mean? [Use] [Cancel])
      if(suggestionBox){
        suggestionBox.innerHTML = '<div style="background:rgba(255,255,255,0.03); padding:10px; border-radius:8px;">Did you mean: <strong>'+fuzzyKey+'</strong>? <button id="useSuggestionBtn" style="margin-left:8px;padding:6px 10px;border-radius:6px;">Use</button> <button id="dismissSuggestionBtn" style="margin-left:6px;padding:6px 10px;border-radius:6px;">Dismiss</button></div>';
        suggestionBox.style.display = 'block';
        document.getElementById('useSuggestionBtn').onclick = function(){
          suggestionBox.style.display = 'none';
          window.showFoodData(fuzzyKey);
        };
        document.getElementById('dismissSuggestionBtn').onclick = function(){
          suggestionBox.style.display = 'none';
        };
      } else {
...
// re-wire search controls
(function ensureWiring(){
  const btn = document.getElementById('searchBtn');
  if(btn) btn.onclick = function(){ const q=document.getElementById('manualFood').value.trim(); if(q) window.lookupFoodByName(q); };
  const input = document.getElementById('manualFood');
  if(input) input.onkeydown = function(e){ if(e.key==='Enter'){ e.preventDefault(); const q=input.value.trim(); if(q) window.lookupFoodByName(q); } };
})();
