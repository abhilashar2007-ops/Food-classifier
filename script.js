...
function isFruitOrVeg(name){
 const keywords=[
  "apple","banana","orange","mango","grape","papaya","pineapple","guava","watermelon",
  "berry","kiwi","pear","peach","plum","pomegranate","fig","date","avocado",
  "tomato","potato","onion","carrot","beetroot","cabbage","cauliflower","spinach",
  "broccoli","capsicum","brinjal","okra","cucumber","pumpkin","radish","lettuce",
  "vegetable","fruit"
 ];
 return keywords.some(k=>name.toLowerCase().includes(k));
}

function classifyFood(n, name){
 if(isFruitOrVeg(name)) return "Healthy";
...
async function searchNutrition(){
 ...
 const category=classifyFood(n, food);

 let adv=[], dis=[];
 adv.push("Naturally rich in essential vitamins and minerals.");
 if(category==="Healthy"){
  adv.push("Supports overall health when consumed regularly.");
 }else{
  dis.push("Consume in moderation as part of a balanced diet.");
 }

 if(n.sugar>10) dis.push("High sugar content may affect blood sugar levels.");
 if(n.fat>15) dis.push("High fat intake may lead to weight gain.");
