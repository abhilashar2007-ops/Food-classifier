...
// Stylish site script: TM + MobileNet + search + UI updates
...
document.getElementById('copyBtn').addEventListener('click', ()=>{
  const name = document.getElementById('foodName').innerText;
  const nutri = document.getElementById('nutri').innerText;
  navigator.clipboard.writeText(name + '\n' + nutri).then(()=>alert('Copied to clipboard'));
});

document.getElementById('downloadBtn').addEventListener('click', ()=>{
  const name = document.getElementById('foodName').innerText;
  const data = {name, nutri: document.getElementById('nutri').innerText, adv: document.getElementById('adv').innerText, disadv: document.getElementById('disadv').innerText};
  const blob = new Blob([JSON.stringify(data, null, 2)], {type:'application/json'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a'); a.href = url; a.download = (name || 'result') + '.json'; document.body.appendChild(a); a.click(); a.remove();
});

function populateSuggestions(){ const dl = document.getElementById('food-suggestions'); Object.keys(foodData).sort().forEach(k=>{ const opt=document.createElement('option'); opt.value=k; dl.appendChild(opt); }); }
